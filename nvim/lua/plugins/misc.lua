return {
	"nvim-lua/plenary.nvim",
	{ "folke/ts-comments.nvim", opts = {}, event = "VeryLazy" },
	{ "karb94/neoscroll.nvim", opts = { duration_multiplier = 0.5 } },
	{ "fladson/vim-kitty", ft = "kitty" },
	{ "axelvc/template-string.nvim", opts = { jsx_brackets = false, remove_template_string = true } },

	{
		"mg979/vim-visual-multi",
		init = function()
			vim.g.VM_theme = "purplegray"
			vim.g.VM_maps = {
				["Goto Prev"] = "",
				["Goto Next"] = "",
				["I Return"] = "",
				["I Down Arrow"] = "",
				["I Up Arrow"] = "",
			}
		end,
	},

	{
		"lewis6991/gitsigns.nvim",
		config = function()
			local gs = require("gitsigns")
			vim.keymap.set("n", "<leader>r", gs.reset_hunk, { desc = "Reset Hunk" })
			vim.keymap.set("n", "<leader>gp", gs.preview_hunk, { desc = "Preview Hunk" })
			gs.setup({ attach_to_untracked = true, current_line_blame = true })
		end,
	},

	{
		"NvChad/nvim-colorizer.lua",
		enabled = true,
		opts = {
			user_default_options = {
				css = true,
				css_fn = true,
				tailwind = "both",
				tailwind_opts = {
					update_names = false,
				},
			},
		},
	},

	{
		"supermaven-inc/supermaven-nvim",
		config = function()
			require("supermaven-nvim").setup({
				keymaps = { accept_suggestion = "<Tab>", accept_word = "<M-Tab>" },
				color = { suggestion_color = "#666666", cterm = 244 },
			})
		end,
	},

	{
		"mrjones2014/smart-splits.nvim",
		build = "./kitty/install-kittens.bash",
		lazy = false,
		config = function()
			vim.keymap.set({ "n", "t" }, "<A-Left>", require("smart-splits").move_cursor_left)
			vim.keymap.set({ "n", "t" }, "<A-Down>", require("smart-splits").move_cursor_down)
			vim.keymap.set({ "n", "t" }, "<A-Up>", require("smart-splits").move_cursor_up)
			vim.keymap.set({ "n", "t" }, "<A-Right>", require("smart-splits").move_cursor_right)
		end,
	},
}
